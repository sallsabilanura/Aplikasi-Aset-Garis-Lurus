        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <!-- Search Form -->
                    <form method="GET" action="{{ route('asets.penghapusan') }}" class="d-flex">
    <!-- Input pencarian -->
    <input type="text" name="search" class="form-control w-50" placeholder="Cari Nama/Kode Aset" value="{{ request('search') }}">
    <button class="btn btn-primary ms-2" type="submit" name="filter" value="search">Cari</button>

    <!-- Dropdown status -->
    <select name="status" class="form-select w-25 ms-2">
        <option value="">-- Semua Status --</option>
        <option value="Aktif" {{ request('status') == 'Aktif' ? 'selected' : '' }}>Aktif</option>
        <option value="Tidak Aktif" {{ request('status') == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
    </select>
    <button class="btn btn-secondary ms-2" type="submit" name="filter" value="status">Cari</button>
</form>
